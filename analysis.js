...
window.Quantra = (function () {
  ...
  function forecast(closes, horizon = 30, bias = 0, opts = null) {
    ...
    // threshold odds for the movers radar: modeled probability of a ±X% move by horizon end
    const probUp10 = ends.filter((e) => e > p0 * 1.10).length / SIMS;
    const probDown10 = ends.filter((e) => e < p0 * 0.90).length / SIMS;
    let probs = null;
    if (opts && Array.isArray(opts.thresholds)) {
      probs = { up: {}, down: {} };
      for (const t of opts.thresholds) {
        const f2 = Math.max(0.1, Math.min(50, +t)) / 100;
        probs.up[t] = ends.filter((e) => e > p0 * (1 + f2)).length / SIMS;
        probs.down[t] = ends.filter((e) => e < p0 * (1 - f2)).length / SIMS;
      }
    }
    // Near-term checkpoint first: band width grows ~sqrt(t), so the h/6 horizon is
    // naturally ~2.4x tighter than the full horizon at the SAME confidence — the honest
    // way to quote a narrower, more precise range.
    const cps = [...new Set([Math.max(1, Math.round(horizon / 6)), Math.max(1, Math.round(horizon / 3)), Math.max(2, Math.round((2 * horizon) / 3)), horizon])];
    ...
    return { p0, horizon, mu, sigma, mid, lo, hi, lo25, hi75, mcMid, probUp, probUp10, probDown10, probs, horizons, regimeScale, calScale,
