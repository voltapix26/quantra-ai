...
function fileStore() {
  ...
  return {
    ...
    async allOrgs() { return Object.values(orgs); },
    ...
    async findOrgByApiKey(k) { return Object.values(orgs).find((o) => o.apiKey === k) || null; },
...
function pgStore() {
  ...
  return {
    ...
    async allOrgs() { const r = await q('select data from orgs'); return r.rows.map((x) => x.data); },
    ...
    async findOrgByApiKey(k) { return one(await q("select data from orgs where data->>'apiKey'=$1", [k])); },
